...
st.markdown("""
    <style>
    /* 리스트 항목을 가로로 정렬하고 버튼을 텍스트 옆에 배치 */
    .row-container { display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #ddd; padding: 8px 0; }
    .text-box { font-size: 11px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex-grow: 1; }
    .btn-box { display: flex; gap: 2px; flex-shrink: 0; }
    /* 버튼 스타일 조정 */
    div.stButton > button { padding: 0px 5px !important; font-size: 10px !important; height: 22px !important; }
    </style>
...
with t_col: st.markdown(f"##### 📋 {len(df)}건", unsafe_allow_html=True)
with b_col1:
    df_ex = df.copy()
...
# 6. 리스트 출력 (HTML/CSS Flex 활용)
for row in df.itertuples():
    st.markdown(f"""
        <div class="row-container">
            <div class="text-box">{row.timestamp} | <b>{row.item_name}</b> | {row.author} (UP:{row.is_update}/DTC:{row.is_dtc})</div>
        </div>
    """, unsafe_allow_html=True)
    # 버튼은 별도 컬럼으로 배치하되 가로로 붙임
    b1, b2 = st.columns([1, 6])
    if b1.button("수정", key=f"e{row.id}"):
        ...
        st.rerun()
    if b2.button("삭제", key=f"d{row.id}"):
